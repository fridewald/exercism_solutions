...
pub fn roll(game: Game, knocked_pins: Int) -> Result(Game, Error) {
  ...
  // TODO: can we improve this instead of heaving early return
  use _ <- result.try(case knocked_pins {
    pins if pins <= 10 && pins >= 0 -> Ok(pins)
  ...
  let games_with_bonus = [
    ...
    |> list.scan(
      ...
      with: fn(following_frame: Frame, current_frame: Frame) {
        // let following_frame = tup.0
        // let current_frame = tup.1
        case current_frame {
          // spare
